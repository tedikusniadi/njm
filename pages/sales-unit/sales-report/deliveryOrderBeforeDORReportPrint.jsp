...
<table width="100%"  cellpadding="4" cellspacing="0" class="report-table">
	...
	<tr>
		<td colspan="8" nowrap="nowrap" style="border-bottom:1px solid black;border-right:1px solid black;border-left:1px solid black;"><strong><c:out value='${report.org.name}'/></strong></td>
 		...
 		<c:forEach items='${report.deliveryOrders}' var='do'>
 		<tr>
			<td nowrap="nowrap" style="border-bottom:1px solid black;border-left:1px solid black;"><fmt:formatDate value='${do.date}' pattern='dd-MM-yyyy'/></td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:out value='${do.code}'/></td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:out value='${do.deliveryPlanningSequence.deliveryPlanning.planable.code}'/></td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:out value='${do.deliveryPlanningSequence.deliveryPlanning.planable.customer.name}'/></td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:out value='${do.deliveryPlanningSequence.deliveryPlanning.planable.customerUnit.name}'/></td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:forEach items="${do.items}" var="item"><c:if test="${item.sequenced}"><c:set var="orderItem" value="${item}"/></c:if></c:forEach>${orderItem.product.typeNames}</td>
	     	<td nowrap="nowrap" style="border-bottom:1px solid black;"><c:forEach items="${do.items}" var="item"><c:if test="${item.sequenced}"><c:set var="orderItem" value="${item}"/></c:if></c:forEach>${orderItem.product.colors}</td>
     		<td nowrap="nowrap" style="border-bottom:1px solid black;border-right:1px solid black;"><c:out value='${do.note}'/></td>
 		</tr>
 		</c:forEach>
	</c:forEach>
    </tbody>
